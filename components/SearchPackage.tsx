...
export default function SearchPackage() {
    ...
    return (
        <form onSubmit={handleSubmit} className='m-auto w-3/6 relative'>
            <Combobox value={selectedPkg} onChange={setSelectedPkg}>
                <Combobox.Input
                    ...
                    placeholder='search...'
                />
                <Combobox.Options className='absolute z-50 bg bg-white divide-y divide-slate-100 cursor-pointer w-full rounded-md overflow-hidden'>
                    {options?.map((opt) => (
                        <Combobox.Option key={opt.package.name} value={opt.package.name} onClick={() => handleSelect(opt.package.name)}>
                            {({ active, selected }) => (
